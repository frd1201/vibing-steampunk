...
// sapClientPattern is the shape of an SAP client number. Compiled once rather
// than on every login.
var sapClientPattern = regexp.MustCompile(`^[0-9]{3}$`)
...
func BrowserLogin(ctx context.Context, sapURL string, insecure bool, timeout time.Duration, execPath string, verbose bool, sapClient string) (map[string]string, error) {
	...
	if sapClientPattern.MatchString(sapClient) {
		q := adtURL.Query()
